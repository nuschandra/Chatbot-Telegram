...
model = keras.models.load_model("bert_intent_detection.hdf5", custom_objects={
                                "BertModelLayer": BertModelLayer}, compile=False)

tokenizer = FullTokenizer(vocab_file="vocab.txt")
classes = ['greetings', 'hiring_request', 'goodbye',
           'interview_schedule', 'schedule_list', 'bot_skill']
print(classes)
            ...
            if final_intent == 'greetings':
                if (database_updates.get_record_by_chat_id_and_date(date_of_msg, chat_id)):
                    responses = random.choice(
                        tg['secondary_responses']).format(first_name)
                else:
                ...
                if (database_updates.get_prev_intent(chat_id) == 'hiring_request'):
                    interview_datetime = slot_detection.schedule_slot_detection(
                        user_text)
                    ...
                    # can be tertiary response
                    responses = "Sorry please upload JD and then invoke this intent"
            elif final_intent == 'schedule_list':
                responses = slot_detection.schedule_list(user_text, chat_id)
            else:
                responses = random.choice(tg['responses'])

    database_updates.insert_chatbot_user_data(
        date_of_msg, first_name, chat_id, final_intent)
    return responses
