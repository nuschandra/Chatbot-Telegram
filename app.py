...
bot_url = "https://629d6553e483.ngrok.io/"
...
def get_schedule(bot, chat_id, sch_list):
    ...
    for cn, objid, sch in sch_list:
        ...
        button_list.append(InlineKeyboardButton(
            cn+" "+sch, callback_data=str(dic)))

    button_list = [button_list[i:i + 1] for i in range(0, len(button_list), 1)]
    reply_markup = InlineKeyboardMarkup(button_list)
    bot.send_message(
        chat_id=chat_id, text='This is your Schedule. Please select the the appointment from the following to confirm or cancel. :', reply_markup=reply_markup)
    return
...
def show_confirm(bot, chat_id, context):
    ...
    reply_markup = InlineKeyboardMarkup(button_list)
    bot.send_message(chat_id=chat_id, text='Confirm or Cancel.',
...
def handle_callback(bot, update):
    ...
    if(context['type'] == "show_confirm"):
        ...
        database_updates.cancel_schedule(context['sce_id'])
            ...
            if file_type != 'application/pdf':
                ...
                for ids in resume_id:
                    ...
                    bot.sendDocument(chat_id=chat_id, document=open(
                        file_to_send, 'rb'), caption=get_candidate_details, reply_markup=InlineKeyboardMarkup(keyboard))
